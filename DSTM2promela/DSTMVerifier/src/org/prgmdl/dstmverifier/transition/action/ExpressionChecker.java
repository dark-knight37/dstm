...
import org.prgmdl.dstmverifier.transition.action.node.AChuseAtomic;
import org.prgmdl.dstmverifier.transition.action.node.ACompareExpr;
import org.prgmdl.dstmverifier.transition.action.node.AEnumExpr;
...
public class ExpressionChecker extends BaseActionAnalyser {
    ...
    public void inAChuseAtomic(AChuseAtomic node) {
	    if (step == true) {
	    	...
	    	node.apply(tf);
	        String idname = tf.getIdentName();
	        if (this.dtt.isDefined(idname)) {
	        	String typename = this.dtt.getTypeName(idname);
		    	this.types.add("Chn[" + typename + "]");
	        } else {
		    	this.types.add(null);
	        }
	    }
